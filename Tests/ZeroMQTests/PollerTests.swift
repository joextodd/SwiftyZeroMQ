...
class PollerTests: XCTestCase {
    ...
    func testRequestReply() throws {
        ...
        XCTAssertEqual(socks[replier],   ZeroMQ.PollFlags.none)
        XCTAssertEqual(socks[requestor], ZeroMQ.PollFlags.pollOut)
        ...
        XCTAssertEqual(socks[requestor], ZeroMQ.PollFlags.none)

        // After a short wait replier should go into .pollIn
        usleep(500)
        socks = try poller.poll()

        XCTAssertEqual(socks[replier], ZeroMQ.PollFlags.pollIn)

        // After receive, replier should go into .pollOut
        try _ = replier.recv()
        socks = try poller.poll()
        XCTAssertEqual(socks[replier], ZeroMQ.PollFlags.pollOut)
    ...
    func testPair() throws {
        ...
        var socks = try poller.poll()
        XCTAssertEqual(socks[s1], ZeroMQ.PollFlags.pollOut)
        XCTAssertEqual(socks[s2], ZeroMQ.PollFlags.pollOut)
        ...
        usleep(500)
        socks = try poller.poll()
        XCTAssertEqual(socks[s1], [ZeroMQ.PollFlags.pollOut, ZeroMQ.PollFlags.pollIn])
        XCTAssertEqual(socks[s2], [ZeroMQ.PollFlags.pollOut, ZeroMQ.PollFlags.pollIn])
